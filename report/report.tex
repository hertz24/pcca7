...
\usepackage{graphicx,amsmath,parskip,cancel,listings,float,caption,titling,booktabs,titlesec}
...
\subsection{Proof}
This following proof can be found in "Faster arithmetic for number-theoretic transforms" page 3 \cite{DBLP:journals/corr/abs-1205-2926} \\
...
For the AVX version, we decided to use two main built-in AVX2 functions that can be found on Intel's Instrinsic Guide: \cite{intel_instrinsics}
\begin{itemize}
...
All excution time were measured using \texttt{prof\_repeat()} from the FLINT library \cite{flint}
...
Just like our AVX implementation, all implementation in NEON require the use of \texttt{vmul\_u32}(...) ( \texttt{\_mm256\_mul\_epu32(...)} equivalence in NEON). Thus by the same reasoning, this cuts the acceleration factor of the SIMD implmenetation by at worse, by half (2x).
...
\end{itemize}
\pointlabel{Note} AMD Zen 4 simulates AVX-512 using 2 register of AVX2. This is not a real AVX-512
...
\begin{figure}[H]
    ...
    \caption{Execution time (in microsecondes) comparison: Standard vs. Unrolled (AVX)}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Execution time (in microsecondes) comparison of all implementation variants (AVX)}
\end{figure}
\begin{table}[H]
    \centering
    \rowcolors{2}{green!10}{white}
...
\pointlabel{Observation}From the graph, we can see that for small vector sizes (under 500), the performance difference between Variant 1 and Variant 2 is negligible. However, for larger vector sizes, Variant 1 actually outperformes slightly.

As for Variant 3, it outperforms both previous variants for all sizes.

On average, Variant is 2.22\% slower than Variant 1, whereas Variant 3 is 24.85\% faster than Variant 1.

\pointlabel{Conclusion}The measured results contradict our hypothesis that both \texttt{mullo\_epi32} variants are faster than \texttt{mul\_epu32}. Even though Variant 2 should in theory outperform the Variant 2 because 8 integers is processed per iteration. In practice, Variant 2 performes worse because its SIMD operation cost is more cycles (1 cycle per operation \texttt{mullo\_epi32}) compared to Variant 1 (0.5 cycle per operation \texttt{mul\_epu32}) according to uops.info \cite{uopsInfoTable}
...
\begin{table}[H]
    ...
    \begin{tabular}{|c|c|c|c|c|c|}
        ...
        \hline
    \end{tabular}
    \caption{Execution time for different implementations in relation to the size of the vector in microsecondes (AVX)}
\end{table}
...
\begin{figure}[H]
    ...
    \caption{Execution time (in microsecondes) comparison: Standard vs. Unrolled (NEON)}
\end{figure}
...
\begin{table}[H]
    ...
    \begin{tabular}{|l|c|c|c|c|c|}
        ...
        \hline
    \end{tabular}
    \caption{Execution time for different implementations in relation to the size of the vector in microsecondes (NEON)}
\end{table}
...
\pointlabel{Observation} From the results table, we firstly notice irregularities between the naive methode and FLINT. The library FLINT on NEON is actually slower.

Even so, both variants implemented with NEON are overall much faster than their FLINT counter part with Variant 1 being 2.4x faster than FLINT's version. Variant 2 being 2.6 times faster.
\pointlabel{Conclusion} he acceleration factors observed for both variants are consistent with the NEON theoretical hypothesis outlined in Section 4.2.
...
\pointlabel{Note} Results from NEON could be misleading as all of the results are of order $10^{-4}$ microsecondes meanwhile results from AVX are of order $10^{-1}$ microseconds. Suppose these results are corrects, the naive modulo operation (\%) would be 1000 times faster than any implementation in AVX2 and AVX-512

\section{Improvements}
\pointlabel{Testings on INTEL}: These results were obtained on AVX architecture built on AMD (Zen 4). Further test and analysis needs to be carried out on the AVX architecture build on Intel as well to improve the validity of the results.
\pointlabel{Extensive testing on NEON}: The results obtained for NEON are inconsistance and does not reflect the theoretical analysis. More test should be carried out on different NEON machine architecture to ensure a better scope of speedup.
\printbibliography

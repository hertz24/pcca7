...
\begin{itemize}
    \item \texttt{\_mm256\_mul\_epu32(...)} : This function treats the input as unsigned 32-bit integers. However, it only multiplies the
          even-indexed slots (0, 2, 4, 6) of the vector to make room for the full 64-bit results.
    \item \texttt{\_mm256\_mullo\_epi32(...)} : This function treats the input as signed 32-bit integers. It multiplies all 8 pairs of numbers but only stores the bottom half of the result.
\end{itemize}


\pointlabel{Variant 1: Intel \texttt{\_mm256\_mul\_epu32(...)} (see Listing 1)}
...
\pointlabel{Variant 2: Intel \texttt{\_mm256\_mullo\_epi32(...)} (see Listing 2)}
...
\pointlabel{Variant 3: Intel (see Listing 3)} \\
...
\end{itemize}
\pointlabel{Variant 1: \texttt{vmull\_u32(...)} (see Listing 7)}
...
\pointlabel{Variant 2: \texttt{vmull\_u32(...)} (see Listing 8)}
...
\end{itemize}
\pointlabel{Note} AMD Zen 4 simulates AVX-512 using two registers of AVX2. This is not a real AVX-512
...
\subsection{AVX2}
\begin{lstlisting}[caption = Variant 1 (AVX2)]
...
\begin{lstlisting}[caption = Variant 2 (AVX2)]
...
\begin{lstlisting} [caption = Variant 3 (AVX2)]
...
\newpage
\subsection{NEON}
\begin{lstlisting} [caption = Variant 1 (NEON)]
...
\end{lstlisting}
\begin{lstlisting}[caption = Variant 2 (NEON)]

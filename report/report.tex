...
For NEON architecture, \textbf{Variant 2: \texttt{\_shoup\_mullo\_neon }} is the best performing implmentation with an acceleration of 2.6x

\pointlabel{Note} Results from NEON could be misleading as all of the results are of order $10^{-4}$ microsecondes meanwhil results from AVX are of order $10^{-1}$ microseconds. Suppose these results are corrects, the naive operation (\%) would be 1000 times faster than any implementation in AVX2 and AVX-512

\section{Improvements}
\pointlabel{Testings on INTEL}: These results were obtained on AVX architecture built on AMD (Zen 4). Further test and analysis needs to be carried out on the AVX architecture build on Intel as well.

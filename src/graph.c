...
void free_data_tab(void)
{
    ...
    Vector *vectors = vector_sample.vectors;
    if (vectors != NULL)
    {
        for (ulong i = 0; i < vector_sample.size; i++)
            free_vector(*(vectors + i));
        free(vectors);
    }
}

static int benchmark(int fd, int scale, ulong nb_points, Parameters param, const AlgorithmID algorithm_ids[], int nb_algo)
{
    if (vector_sample.vectors == NULL)
    {
        vector_sample.vectors = malloc(nb_points * sizeof(Vector));
        vector_sample.size = nb_points;
        if (vector_sample.vectors == NULL)
        {
        ...
        for (ulong i = 0; i < nb_points; i++)
            *(vector_sample.vectors + i) = rand_vector((i + 1) * scale);
    }
    for (int i = 0; i < nb_algo; i++)
    {
        ...
        if (*(data_tab + id) == NULL)
        {
            *(data_tab + id) = malloc(nb_points * sizeof(double));
            ...
            for (ulong j = 0; j < nb_points; j++)
                *(*(data_tab + id) + j) = time_algorithm(algorithms[id].address, param, *(vector_sample.vectors + j));
        }
